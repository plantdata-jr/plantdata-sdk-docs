...
import { PdSDKPluginStatsCluster, PdSDKPluginStatsClusterSettings } from '../plugins/stats-cluster/stats-cluster';
...
import { PdSDKNetChartPluginSettings } from '../../netchart';
import { PdSDKNetChartData, PdSDKNetChartDataNode } from '../../netchart/model';
import { PdSDKAjaxSettings } from '../../utils/service';
import { PdVisChartLoadParams } from '../../../vis';
/**
 * SDK 网络关系图配置
 */
export interface PdSDKClusterSettings<T extends PdVisNetChartSettings> extends PdSDKNetChartAnalysisSettings<T> {
    ...
    /**
     * 节点双击事件
     * @param {PdSDKNetChartDataNode} node 双击的节点
     */
    onNodeDblClick?: (node: PdSDKNetChartDataNode) => void;
    /**
     * 插件通用配置
     */
...
export declare abstract class PdSDKCluster<T extends PdVisNetChart, V extends PdVisNetChartSettings> extends PdSDKNetChartAnalysis<T, V> {
     ...
     */
    protected constructor(settings: PdSDKClusterSettings<V>);
    /**
     * 销毁实例
     */
    destroy(): void;
     ...
     */
    protected transformData(data: any): PdSDKNetChartData;
}
